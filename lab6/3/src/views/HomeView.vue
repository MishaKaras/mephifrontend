...
import { getTasks, deleteTask, toggleTask } from '../store/tasks.js'
...
<template>
    ...
    <p v-if="tasks.length === 0" class="empty">Задач пока нет. <RouterLink to="/add">Добавить?</RouterLink></p>
    ...
    <ul v-else class="task-list">
        ...
        <div class="task-actions">
            ...
            <button @click="handleToggle(task.id)">
            {{ task.completed ? '↩ Вернуть' : '✅ Выполнено' }}
            </button>
            <button class="btn-delete" @click="handleDelete(task.id)">&#128465; Удалить</button>
        </div>
